...
def test_create_pr(client):
    team_data = make_team(
        "backend", [("u1", "Alice", True), ("u2", "Bob", True), ("u3", "Charlie", True)]
    )
...
def test_create_duplicate_pr(client):
    ...
    response = client.post(
        "/pullRequest/create",
        json={"pull_request_id": "pr1", "pull_request_name": "Fix2", "author_id": "u1"},
    )
...
def test_reassign_non_assigned_reviewer(client):
    ...
    response = client.post(
        "/pullRequest/reassign", json={"pull_request_id": "pr1", "old_user_id": "u4"}
    )
    assert response.status_code == 409
...
def test_reassign_on_merged_pr(client):
    ...
    client.post("/pullRequest/merge", json={"pull_request_id": "pr1"})
    response = client.post(
        "/pullRequest/reassign", json={"pull_request_id": "pr1", "old_user_id": "u2"}
    )
    assert response.status_code == 409
...
def test_inactive_users_not_assigned(client):
    team_data = make_team(
        "backend",
        [("u1", "Alice", True), ("u2", "Bob", False), ("u3", "Charlie", True)],
    )
    client.post("/team/add", json=team_data)
    response = client.post(
        "/pullRequest/create",
